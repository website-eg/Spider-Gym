...
const authenticateToken = (req, res, next) => {
  ...
  if (!token) return res.status(401).json({ message: 'يجب تقديم رمز الدخول (التوكن)' });

  jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
    if (err) return res.status(403).json({ message: 'رمز الدخول غير صالح أو منتهي' });
    req.user = user;
    next();
  });
};
...
app.post('/admin/register', async (req, res) => {
  ...
  try {
    ...
    if (existingAdmin) return res.status(400).json({ message: 'اسم المستخدم موجود مسبقًا' });
...
app.put('/admin/trainers/:id', authenticateToken, async (req, res) => {
  const { name, specialty, image } = req.body;
  try {
    const updatedTrainer = await Trainer.findByIdAndUpdate(req.params.id, { name, specialty, image }, { new: true });
    if (!updatedTrainer) return res.status(404).json({ message: 'المدرب غير موجود' });
...
app.delete('/admin/trainers/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!deletedTrainer) return res.status(404).json({ message: 'المدرب غير موجود' });
...
app.post('/admin/subscriptions', authenticateToken, async (req, res) => {
  ...
  try {
    const newSubscription = new Subscription({ title, price, duration, description });
...
app.put('/admin/subscriptions/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!updatedSubscription) return res.status(404).json({ message: 'الاشتراك غير موجود' });
    res.json({ message: 'تم تحديث الاشتراك', subscription: updatedSubscription });
  } catch {
...
app.delete('/admin/subscriptions/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!deletedSubscription) return res.status(404).json({ message: 'الاشتراك غير موجود' });
...
app.put('/admin/news/:id', authenticateToken, async (req, res) => {
  const { title, content, image } = req.body;
  try {
    const updatedNews = await News.findByIdAndUpdate(req.params.id, { title, content, image }, { new: true });
...
app.put('/admin/testimonials/:id', authenticateToken, async (req, res) => {
  const { name, message, image } = req.body;
  try {
    const updatedTestimonial = await Testimonial.findByIdAndUpdate(req.params.id, { name, message, image }, { new: true });
    if (!updatedTestimonial) return res.status(404).json({ message: 'رأي العميل غير موجود' });
    res.json({ message: 'تم تحديث رأي العميل', testimonial: updatedTestimonial });
  } catch {
...
app.delete('/admin/testimonials/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!deletedTestimonial) return res.status(404).json({ message: 'رأي العميل غير موجود' });
...
app.delete('/admin/contacts/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (!deletedContact) return res.status(404).json({ message: 'الرسالة غير موجودة' });
